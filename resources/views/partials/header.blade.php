...
<nav class="flex justify-between flex-row p-3 border-b border-gray-300 shadow-sm">
    ...
    <div class="flex items-center">
        <!-- Clickable ONEUTM -->
        <a href="{{ route('dashboard') }}" class="text-primary font-bold text-2xl mr-0 lg:mr-32">
            ONEUTM
        </a>
    ...
    <!-- Right Section with Alpine.js for Dropdown -->
    <div x-data="{ open: false }" class="text-secondary inline-flex items-center relative">
        ...
        <button @click="open = ! open" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-user mr-2">
        ...
        <!-- Dropdown menu -->
        <div x-show="open" x-transition @click.away="open = false" class="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
            <div class="py-1">
